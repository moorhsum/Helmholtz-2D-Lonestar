...
double XDER2(double **level, int i, int j, int mode)
{
	...
	if (mode == 1)	//	high order finite differencing with attempt to deal with kinks, would cause asymmetry	//
	{
		if (j < 0)
...
//				return DDxc;
	...
	else	//	Straight up second order finite differencing	//
	{
		if (j < 0)
			return (level[i][abs(j)+1] - 2.0 * level[i][abs(j)] + level[i][abs(j)-1])/(H*H);
		if (j == 0)
...
double YDER2(double **level, int i, int j, int mode)
{
	...
	if (mode == 1)	//	With attempts to accommodate kinks, but causes asymmetry	//
	{
